...
Smart Capture는 다음처럼 더 구체적인 기준을 갖는다. 수치는 프로젝트 기록에 남은 조건 기준으로만 해석한다.

| 단계 | 기준 |
| --- | --- |
| 전처리 | 화면을 가로 640 기준으로 resize하고 grayscale로 변환 |
| 특징점 추출 | ORB로 2,000개 이내 특징점을 사용 |
| 노이즈 완화 | 여러 프레임 동안 같은 위치에 유지되는 특징을 더 신뢰 |
| 슬라이드 구간 판단 | 유효 특징이 50개를 넘으면 슬라이드 후보 구간으로 판단 |
| 대표 프레임 선택 | 특징점이 가장 많은 프레임을 캡처 후보로 갱신 |
| 전환 판단 | 특징 점수가 최대치 대비 15% 이상 줄면 전환으로 판단 |

이 기준의 목적은 화면 전체 픽셀을 무겁게 비교하는 것이 아니라, 슬라이드 안의 정보량과 구조 변화를 빠르게 추적하는 것이다. 강사의 움직임, 포인터, 작은 애니메이션은 가능한 한 노이즈로 보고, 실제 학습 정보가 바뀌는 순간을 찾는 데 초점을 둔다.

중복 제거는 두 단계로 나눠 보는 것이 안전했다.

| 중복 제거 단계 | 역할 |
| --- | --- |
| pHash | 전체적인 유사도를 빠르게 확인 |
| ORB matching | 통과한 후보의 세부 구조를 다시 비교 |

중복 슬라이드로 판단되면 새 파일을 계속 만들기보다 시간 정보를 병합한다. 이렇게 하면 저장 공간과 VLM 입력량을 줄이면서도 같은 슬라이드가 영상에서 어느 구간에 나타났는지 추적할 수 있다.
...
VLM은 Qwen 계열 모델을 사용해 캡처된 슬라이드의 텍스트, 표, 수식, 다이어그램 관계를 구조화하는 역할을 맡았다. 단일 이미지 요청만 반복하면 병목이 커지므로, 여러 캡처 이미지를 batch 단위로 묶어 처리하는 방향도 함께 정리됐다. 이 역시 전체 서비스 성능 주장이라기보다 VLM 병목을 줄이기 위한 설계 선택으로 보는 것이 맞다.
...
이렇게 분리하면 요약 모델이 핵심과 보조 정보를 섞어 쓰는 위험을 줄일 수 있다. VLM에서 객체명, 수량, 레이아웃 설명이 흔들리면 요약도 흔들린다. 따라서 prompt 개선의 목표는 절대 성능 주장이 아니라 오류 가능성을 줄이는 방향의 입력 품질 개선이었다.
...
- 이전 글: [05. 아키텍처: STT, VLM, Fusion을 연결하는 방법]({% post_url 2025-12-16-project-sesac-note-05-architecture %})
- 다음 글: [07. 비동기 처리: 긴 영상의 대기시간과 상태 추적 줄이기]({% post_url 2026-01-06-project-sesac-note-07-async-pipeline %})
