...
![PDF OCR layout label code note](/assets/images/study/code-notes/study-code-pdf-ocr-layout.png)

이 코드 조각에서 가장 중요하게 본 부분은 OCR 호출이 아니라 `CLASS_LABELS`였다. 문서를 읽기 전에 title, plain text, figure, table, formula처럼 block type을 먼저 고정한다. 그래야 뒤에서 표는 표대로, 그림은 caption과 함께, 본문은 문단 단위로 다룰 수 있다.
