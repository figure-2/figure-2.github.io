...
SeSAC:Note에서 긴 영상 처리는 파이프라인 설계의 핵심 병목이었다. 이 글에서는 병목을 어떻게 나누고, 상태 추적과 체감 대기시간을 어떻게 다뤘는지 정리한다.
...
v2.1 단계에서는 prompt와 출력 형식을 다듬어 입력 token을 약 12% 줄인 기록도 정리되어 있다. 이 수치는 프로젝트 기록 기준이며, 전체 비용 절감을 보장하는 값이 아니라 prompt compacting이 병목 완화에 기여할 수 있음을 보여주는 제한된 근거로 봐야 한다.
