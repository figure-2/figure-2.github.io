...
발표 정리 자료 기준으로 최적화 흐름은 네 단계로 정리할 수 있다.

| 단계 | 초점 | 해석 |
| --- | --- | --- |
| v1 | 기능 구현 중심 파이프라인 | 먼저 업로드, 분석, 요약 흐름을 연결 |
| v2 | 품질과 메모리 최적화 | batch 처리와 context 관리로 긴 영상 처리 안정화 |
| v2.1 | 비용과 속도 개선 | prompt와 출력 형식 조정, VLM 호출량 감소 |
| v3 | 비동기 아키텍처와 DB 기반 구조 | STT/capture 병렬화, 단계별 결과 저장, 상태 추적 강화 |

이 타임라인은 성능 수치를 과시하기 위한 것이 아니라, 병목을 발견한 뒤 구조를 어떻게 바꿨는지 보여주는 근거다.
...
v2.1 단계에서는 prompt와 출력 형식을 다듬어 입력 token을 약 12% 줄인 기록도 정리되어 있다. 이 수치는 발표 정리 자료 기준이며, 전체 비용 절감을 보장하는 값이 아니라 prompt compacting이 병목 완화에 기여할 수 있음을 보여주는 제한된 근거로 봐야 한다.
...
외부 API 호출 안정성도 별도 설계 대상이었다. 발표 정리 자료에는 특정 호출 경로에 문제가 생겨도 전체 처리가 바로 중단되지 않도록 round-robin 방식의 key routing을 도입한 흐름이 나온다. 공개 글에서는 이를 운영 성공이 아니라 외부 API 의존성을 낮추기 위한 장애 완화 설계로만 표현한다.
