...
LLM 서비스 평가를 정리하면서 남은 생각은 “평가 지표가 많다”가 아니었습니다. 같은 출력도 무엇을 보려는지에 따라 지표가 달라진다는 점이었습니다.

요약 품질, 업무 리포트 품질, RAG 답변의 근거성을 볼 때는 평가 질문이 다릅니다.
...
ROUGE는 생성 결과와 기준 답변의 단어 overlap을 봅니다.
...
메일 요약이나 문서 요약처럼 기준 요약문이 있는 경우에는 빠른 비교 지표로 쓸 수 있습니다. 하지만 ROUGE가 높다고 답변이 항상 좋은 것은 아닙니다.

## BERTScore

BERTScore는 embedding 기반으로 의미 유사도를 봅니다.
...
여기서는 ROUGE를 표면 비교, BERTScore를 의미 비교에 가깝게 이해했습니다.

## G-Eval

G-Eval은 LLM을 평가자로 쓰는 방식입니다. 요약, 리포트, QA처럼 기준이 복합적인 결과를 볼 때 유용합니다.
...
G-Eval은 사람이 채점하듯 rubric을 만들 수 있다는 점이 장점입니다. 대신 judge 모델이 항상 공정하다고 믿으면 안 됩니다.

## 지표를 함께 읽기

정리하면서 가장 설득력 있었던 방식은 개별 요약은 ROUGE와 BERTScore로 보고, 최종 리포트는 G-Eval로 비교하는 흐름이었습니다.
...
지표는 하나만 고르는 것이 아니라 출력 유형에 맞게 조합해야 합니다.

## 평가 수치를 쓸 때의 주의

평가 수치는 글에 넣기 좋지만 조심해서 써야 합니다.
...
예를 들어 어떤 조건에서 G-Eval 점수가 가장 높았다는 기록이 있어도, 그것은 해당 데이터와 rubric 안에서의 결과입니다. 일반적인 서비스 품질로 바로 확장하면 안 됩니다.

## 평가 지표를 config로 분리하는 코드

평가 코드를 직접 적어보면 지표를 함수 안에 고정하지 않고 config에서 선택하는 편이 자연스럽습니다. 같은 요약 결과라도 ROUGE, BERTScore, G-Eval은 보는 관점이 다릅니다. 그래서 평가 코드는 “하나의 점수 계산기”보다 “여러 평가 질문을 실행하는 runner”에 가깝습니다.
...
평가 지표를 이렇게 나누면 결과 해석도 같이 달라집니다. ROUGE가 낮아도 의미가 맞을 수 있고, G-Eval이 높아도 judge 기준이 느슨하면 위험합니다.

## 지표보다 평가 질문

LLM 서비스 평가는 “정답률” 하나로 설명하기 어렵습니다. 요약은 ROUGE/BERTScore, 리포트는 G-Eval, RAG는 groundedness를 함께 봐야 합니다.

지표를 고르기 전에 평가 질문을 정해야 합니다. “무엇을 좋은 출력이라고 볼 것인가”가 정해져야 점수도 의미를 가집니다.
