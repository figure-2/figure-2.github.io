...
Metadata Weekly는 이를 "2026 AI Reality Check"라고 불렀습니다. **문제는 모델이 아니라 기반(Foundation)입니다.** 그리고 그 기반의 핵심은 데이터입니다.
...
RAG는 마법이 아닙니다. RAG의 품질은 **검색 품질**에 의존하고, 검색 품질은 **데이터 품질**에 의존합니다. Atlan은 이를 명확히 짚습니다. "데이터 품질 문제는 모델 문제가 아니라 컨텍스트 문제다." RAG가 안 되는 이유를 정리하면 다음과 같습니다.
...
**대부분의 문제는 1단계에서 발견됩니다.** 모델이나 프롬프트를 고치기 전에 데이터를 먼저 봐야 합니다.
...
AI 프로젝트의 첫 번째 단계는 코드를 쓰는 것이 아니라 **데이터를 파악하는 것**입니다.
...
모델은 매달 더 좋아지고, 프롬프트 기법도 계속 새로 나옵니다. 하지만 데이터가 준비되지 않으면 이 모든 발전은 무의미합니다.
...
AI를 잘 쓰려면 개발을 잘해야 하고, 개발을 잘하려면 데이터를 잘 다뤄야 합니다. **DX → Data → AX**, 이 순서는 건너뛸 수 없습니다.
