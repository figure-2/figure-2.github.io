...
LLM 서비스를 처음 볼 때는 모델 호출 API를 중심으로 생각했습니다. 그러나 작은 실습으로 흐름을 다시 만들어보니 실제 차이는 모델보다 `데이터 흐름`, `상태 관리`, `저장소 분리`, `평가 위치`에서 더 크게 났습니다.

같은 LLM을 사용해도 원문을 어떻게 보관하는지, 검색 결과를 어떻게 남기는지, 평가를 어느 단계에서 수행하는지에 따라 서비스의 신뢰도가 달라집니다.

## 전체 구조를 나눈다

LLM 서비스는 단일 함수가 아니라 여러 단계의 pipeline입니다. 특히 문서 기반 서비스나 Agent형 서비스에서는 ingestion, indexing, retrieval, generation, evaluation이 서로 다른 책임을 가집니다.
...
실습하면서 계속 되돌아간 질문은 “어떤 데이터를 어느 저장소에 남길 것인가”였습니다.
...
이렇게 나누면 장애가 났을 때 어디가 문제인지 추적하기 쉽습니다. 반대로 모든 것을 하나의 JSON이나 하나의 vector DB에 몰아넣으면 나중에 평가와 디버깅이 어려워집니다.

## pipeline은 동기 요청으로만 처리하지 않는다

짧은 질문-답변 서비스는 API 요청 안에서 바로 처리할 수 있습니다. 하지만 PDF 처리, 이미지 분석, 대량 문서 embedding, Agent workflow처럼 오래 걸리는 작업은 다르게 봐야 합니다.
...
실습 기준으로는 API와 worker를 분리하는 순간부터 `job_id`, `status`, `retry_count`, `error_reason`이 필요해집니다. 화면에는 결과만 보이지만 실제 서비스에서는 처리 상태가 사용자 경험을 결정합니다.

## 저장소를 역할별로 분리한다

RAG나 문서 기반 LLM 서비스에서 자주 헷갈리는 부분은 vector DB의 역할입니다. vector DB는 검색을 위한 index이지 전체 서비스 상태를 책임지는 DB가 아닙니다.
...
이 구조를 잡아두면 evaluation도 쉬워집니다. 검색 실패인지, generation 실패인지, source parsing 실패인지 분리해서 볼 수 있기 때문입니다.

## evaluation은 나중에 붙이는 기능이 아니다

처음에는 evaluation을 완성 후 점검 단계로 생각했습니다. 그러나 구현 관점에서는 evaluation도 architecture의 일부였습니다.
...
특히 RAG에서는 “답변이 틀렸다”만 보면 원인을 알 수 없습니다. 문서가 잘못 잘렸는지, 검색이 실패했는지, LLM이 근거 밖으로 확장했는지를 나눠서 봐야 합니다.

## trace를 남겨야 개선할 수 있다

LLM 서비스는 같은 입력처럼 보여도 prompt, model, retrieval setting, chunk version에 따라 결과가 달라집니다. 그래서 실습할 때도 최소한 다음 값은 남기는 편이 좋습니다.
...
이 정보가 없으면 체감 평가만 남습니다. 숫자를 기록해도 어떤 조건의 숫자인지 알 수 없으면 나중에 다시 설명하기 어렵습니다.

## trace가 남는 pipeline 코드

서비스 전체를 한 번에 만들기보다 trace가 남는 작은 pipeline부터 적어보는 편이 이해가 빨랐습니다.
...
핵심은 함수 이름이 아닙니다. 검색, 생성, 평가, trace를 한 덩어리로 뭉개지 않는 것입니다.
...
LLM 서비스 아키텍처는 모델을 잘 호출하는 구조가 아니라 입력에서 답변까지의 흐름을 추적 가능하게 만드는 구조입니다.

제가 남긴 판단은 단순합니다. 원본은 보존하고, 상태는 DB에 남기고, 검색은 index로 분리하며, 생성 결과는 근거와 함께 저장하고 평가는 pipeline 안에 넣습니다.

이 기준이 있어야 RAG, Agent, PDF/OCR, 평가 지표를 각각 따로 배운 내용이 하나의 서비스 설계로 연결됩니다.

다음 글에서는 공부한 개념을 어떻게 개인 실습 기준으로 바꿨는지 정리한다.

다음 글: [AI 서비스 구현 실습 회고: 개념을 설계 기준으로 바꾸기]({% post_url studies/ai-engineering/2026-06-17-study-ai-service-learning-synthesis-practice %})
