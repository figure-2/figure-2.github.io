...
공부한 내용을 주제별로만 모아두면 금방 흩어졌습니다. 그래서 이번에는 `개념을 어떻게 실습 가능한 설계 기준으로 바꿀 수 있는지`에 맞춰 다시 묶어봤습니다.

처음에는 RAG, OCR, Agent, 평가 지표를 각각 따로 봤습니다. 그러나 작은 예제를 직접 만들어보면 질문이 바뀝니다. “이 개념이 무엇인가”보다 “이 개념을 어디에 붙여야 실패를 줄일 수 있는가”가 더 중요해집니다.

## 학습 주제를 볼 때의 순서

새로운 주제를 볼 때 모델명부터 보면 길을 잃기 쉬웠습니다. 더 오래 남는 것은 문제 정의, 데이터 흐름, 평가 기준이었습니다.
...
이 순서를 잡으면 기술 이름에 끌려가지 않습니다. 같은 RAG라도 문서 처리형 RAG인지, 금융 근거형 RAG인지, Agent가 붙은 RAG인지에 따라 봐야 할 기준이 달라집니다.

## 개념을 구현 질문으로 바꾼다

개념을 읽는 것과 구현해보는 것은 다릅니다. 직접 만들어볼 때는 개념을 질문으로 바꿔야 합니다.
...
이렇게 바꾸면 글을 쓸 때도 단순 요약이 아니라 “제가 무엇을 확인했는지”가 드러납니다.

## 직접 만들어볼 때 남길 산출물

학습 정리 글이 설득력을 가지려면 읽은 내용만으로는 부족했습니다. 작게라도 직접 만져본 흔적이 있어야 나중에 다시 읽을 가치가 생깁니다.
...
이때 코드를 길게 붙이는 것보다 학습용으로 재구성한 짧은 코드가 더 읽기 쉽습니다. 중요한 것은 이름이 아니라 설계 판단입니다.

## 좋은 실습 코드는 작고 재현 가능해야 한다

실습 코드는 길다고 좋은 것이 아닙니다. 하나의 개념을 보여주고 입력과 출력이 명확해야 합니다.

예를 들어 RAG 평가를 공부했다면 전체 서비스를 만들기보다 다음 정도의 작은 단위가 더 낫습니다.
...
이 코드는 완성된 평가 시스템이 아닙니다. 하지만 제가 배운 핵심을 분명히 보여줍니다. RAG 실패를 하나로 보지 않고 retrieval failure와 generation failure로 나눠야 한다는 점입니다.

## 글로 남길 때 조심할 표현

개인 학습 글에서는 표현을 조심해야 합니다. 직접 확인한 범위를 넘겨 쓰면 학습 기록이 아니라 성과 주장처럼 보입니다.
...
이 글의 목적은 성과 주장보다 학습 증거를 남기는 것입니다. 그래서 수치가 있더라도 반드시 조건을 붙이고, 확인하지 않은 성공 claim은 쓰지 않습니다.
...
이 순서가 중요한 이유는 단순합니다. 개념을 많이 아는 것보다 어떤 상황에서 어떤 기준으로 선택할지 아는 것이 실습에 더 가깝기 때문입니다.
...
이 표가 이번 학습에서 가장 오래 남은 산출물입니다. 각 기술을 외우는 대신 다음 구현에서 던질 질문 목록으로 바꿨기 때문입니다.

## 마지막에 남은 기준

여러 내용을 걷어내고 나니 마지막에 남은 것은 기준표였습니다.

이름을 덜어내면 오히려 문제, 데이터, pipeline, 평가, 한계가 더 잘 보입니다.

자료를 따라 적는 데서 멈추지 않고 반복해서 등장한 설계 판단을 제 언어와 코드로 다시 적어보는 것, 이 시리즈는 그 연습에 가깝습니다.

이전 글: [LLM 서비스 아키텍처 실습 정리: Pipeline, Storage, Evaluation]({% post_url studies/ai-engineering/2026-06-17-study-llm-service-architecture-patterns-practice %})
