...
AI Agent를 실제 서비스로 만들 때는 모델 호출보다 운영 구조가 더 중요해집니다. Memory, RAG, guardrails, protocol, cost control이 붙는 순간 Agent는 단순 챗봇이 아니라 상태를 가진 실행 시스템이 됩니다.
...
Agent memory는 단순히 대화 로그를 저장하는 것이 아닙니다. 현재 작업에 필요한 짧은 기억과 세션을 넘어 유지해야 하는 장기 기억을 구분해야 합니다.
...
좋은 memory 설계는 "무엇을 기억할 것인가"보다 "무엇을 잊을 것인가"를 함께 정의합니다. 모든 기록을 넣으면 context noise가 커지고 중요한 정보가 묻힙니다.

## Agentic RAG vs Traditional RAG

Traditional RAG는 보통 한 번 검색하고 한 번 답합니다. Agentic RAG는 검색 결과를 평가하고, 부족하면 query를 바꾸거나 다른 source를 찾아 다시 검색합니다.
...
Agentic RAG는 더 강력하지만 비용과 latency가 증가합니다. 단순 QA에는 Traditional RAG가 더 안정적일 수 있습니다.

## Guardrails

가드레일은 하나의 필터가 아니라 계층적 방어 구조로 보는 편이 적합합니다.
...
특히 tool guardrail이 중요합니다. Agent가 외부 API, DB, 파일 시스템, 결제, 발송 기능을 호출할 수 있다면 답변 품질 문제가 아니라 실제 행동 위험이 생깁니다.

## Cost Control

Agent loop는 단일 LLM 호출보다 token 사용량이 크게 늘 수 있습니다. tool schema, memory, retrieved context, intermediate reasoning, retry가 모두 비용으로 이어집니다.
...
비용 최적화는 품질을 낮추자는 뜻이 아닙니다. 비싼 모델을 써야 하는 구간과 그렇지 않은 구간을 분리하자는 뜻입니다.

## MCP와 A2A

Agent ecosystem에서는 도구와 데이터 연결, agent 간 통신을 표준화하려는 흐름이 있습니다. 대표적으로 MCP와 A2A를 나눠 볼 수 있습니다.
...
MCP는 tool integration 문제를 줄이는 데 초점이 있고, A2A는 여러 agent가 협업할 때의 통신 방식을 다룹니다. 둘은 경쟁 관계라기보다 서로 다른 계층을 담당하는 보완 관계로 이해하는 편이 자연스럽습니다.

## 정리

운영 가능한 Agent는 `LLM + prompt`로 끝나지 않습니다. memory는 상태를 유지하고, RAG는 외부 지식을 가져오며, guardrails는 행동 범위를 제한하고, cost control은 반복 실행이 비용 폭주로 이어지지 않게 막습니다.

이 네 요소 중 하나라도 빠지면 데모는 가능해도 운영 안정성은 낮아집니다.
