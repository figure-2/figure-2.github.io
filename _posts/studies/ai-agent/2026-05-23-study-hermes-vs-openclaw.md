...
저도 그렇게 생각했습니다. OpenClaw를 설치하고 Docker로 실행한 뒤 Telegram 채널을 연결했습니다. 한동안은 잘 돌아갔습니다.

그러나 사용하다 보니 질문이 "어떻게 연결하지?"에서 **"어떻게 더 똑똑하게 만들지?"로** 바뀌었습니다. OpenClaw는 게이트웨이를 안정적으로 운영하는 데 최적화되어 있었고, 제가 원한 것은 에이전트가 스스로 성장하는 구조였습니다. 그래서 Hermes Agent로 넘어왔습니다.

두 프로젝트를 직접 클론해 소스코드를 읽었습니다. README 비교가 아니라 **코드에서 드러나는 설계 철학의 차이**를 정리합니다. 결론부터 말하면, 둘 다 "영구 메모리 + 도구 통합 + 셀프 호스팅"이라는 목표를 두지만 **이를 실현하는 방식은 근본적으로 다릅니다.**

---

## 핵심: Agent-first vs Gateway-first

두 프로젝트의 차이를 한 줄로 압축하면 다음과 같습니다.
...
이 차이는 작아 보일 수 있습니다. 그러나 코드를 열어보면 **이 한 가지 선택이 메모리 구조, 도구 등록 방식, 실행 모델, 확장 방향을 모두 결정합니다.**
...
게이트웨이(Telegram, Discord 등)는 `gateway/` 디렉토리에 있지만 에이전트에게 메시지를 **배달하는 역할**에 머뭅니다. 에이전트가 중심이고 채널은 입출력 표면입니다.
...
에이전트 실행은 Gateway가 받은 메시지를 `agentCommand`로 라우팅해 처리합니다. Gateway가 중심이고 에이전트는 실행 엔진입니다.

이는 단순한 구조적 차이가 아닙니다. **중심이 어디에 있느냐가 확장 방향을 결정합니다.** Hermes는 에이전트를 더 똑똑하게 만드는 방향으로 진화하고(스킬, Curator, MoA), OpenClaw는 연결과 운영을 더 안정적으로 만드는 방향으로 진화합니다(채널 플러그인, 세션 관리, 샌드박스 정책). 같은 기능을 추가하더라도 설계의 무게중심은 다릅니다.

---

## 메모리: 성능 vs 투명성

메모리 시스템은 두 프로젝트의 설계 철학이 가장 선명하게 갈리는 지점입니다. 제가 OpenClaw에서 Hermes로 넘어온 결정적 이유이기도 합니다.
...
과거 대화 검색에는 `session_search` 도구가 SQLite FTS5 인덱스를 사용합니다. 여기에 8개 외부 메모리 플러그인(Honcho, Mem0, RetainDB 등)으로 시맨틱 검색이나 지식 그래프를 연결할 수 있습니다.
...
OpenClaw의 원칙은 단순합니다. **"메모리는 그냥 마크다운."** 숨겨진 상태가 없고, 파일을 열면 에이전트가 무엇을 기억하는지 그대로 보입니다. 오늘과 어제의 daily log만 자동으로 로드하며, 나머지는 인덱싱해 검색할 수 있습니다.
...
정리하면 Hermes는 **"성능 먼저, 캐시를 깨지 마라"**, OpenClaw는 **"투명성 먼저, 사람이 읽을 수 있어야 한다"**에 가깝습니다. 어느 쪽이 맞고 틀린 문제는 아닙니다. 장기 실행 비용이 중요하면 Hermes의 frozen snapshot이 현실적이고, 에이전트가 무엇을 기억하는지 직접 확인하고 싶다면 OpenClaw의 plain markdown이 편합니다.
...
Hermes의 도구 등록은 단순합니다. `tools/registry.py`의 `discover_builtin_tools()` 함수가 `tools/*.py` 파일을 **AST로 스캔**해 `registry.register()` 호출을 자동으로 찾습니다. 도구를 추가하려면 Python 파일을 만들고 register를 호출하면 됩니다.

95개 이상의 빌트인 도구 모듈이 있습니다. 파일, 터미널, 웹, 브라우저, 비전, 코드 실행, 음악, 이미지, 홈 오토메이션을 포함합니다. Toolset으로 카테고리 단위 활성화·비활성화가 가능하고, 도구별 결과 크기 제한도 있습니다.

### OpenClaw — 매니페스트 기반 플러그인

OpenClaw는 도구를 **플러그인 확장**으로 구현합니다. `src/plugin-sdk/`(498 파일)가 공개 계약을 정의하고, 각 도구는 매니페스트 메타데이터로 역량을 선언합니다.
...
Hermes는 **"빠르게 도구를 추가하고 돌려보자"**에, OpenClaw는 **"도구의 권한과 생명주기를 제어하자"**에 최적화되어 있습니다. 프로토타이핑에는 Hermes가 빠르고, 프로덕션 정책 관리에는 OpenClaw의 Slot + Boundary가 견고합니다.

---

## 스킬 시스템: Hermes가 "성장하는 에이전트"인 이유

여기가 결정적 차이입니다. OpenClaw에는 이에 해당하는 시스템이 **없습니다.**
...
Hermes는 복잡한 작업을 완료한 뒤 **에이전트가 자율적으로 스킬 문서를 생성합니다.** 여기에는 트리거 조건, 단계별 지침, 함정, 검증 방법이 포함됩니다. 91개 빌트인 스킬과 agentskills.io 커뮤니티 스킬 520개 이상을 제공합니다.
...
이것이 Hermes를 사용하며 느낀 가장 큰 차이입니다. **작업 경험이 재사용 가능한 절차적 지식으로 결정화됩니다.** 같은 유형의 작업을 반복할수록 에이전트가 실제로 빨라집니다. "AI가 학습한다"는 마케팅 문구가 아니라 스킬 문서라는 형태로 실체가 있습니다.
...
복잡한 자동화를 여러 전문 에이전트에 분배하려면 Hermes가, 하나의 에이전트가 차근차근 밀고 나가는 방식에는 OpenClaw가 적합합니다.

---

## 채널 & 게이트웨이

둘 다 메시징 플랫폼 통합을 지원하지만 접근 방식은 다릅니다.
...
채널 연결이 핵심 가치인 프로젝트답게 OpenClaw의 WebSocket 프로토콜은 정교합니다. `connect` → 핸드셰이크 → nonce 서명 → `hello-ok` + 기능 목록의 구조화된 흐름과 스코프(`read`, `write`, `admin`, `approvals`) 기반의 세밀한 접근 제어를 제공합니다. 이 부분은 OpenClaw가 앞섭니다.

---

## MCP: 클라이언트 vs 양방향

Hermes는 **강력한 MCP 클라이언트**입니다. 3가지 전송 모드(stdio, HTTP/StreamableHTTP, SSE), 전용 비동기 이벤트 루프, 서버별 병렬 도구 호출, 샘플링 지원, 자동 재연결을 제공합니다.

OpenClaw는 한 발 더 나아가 **자체가 MCP 서버로도 작동합니다.** `openclaw mcp serve`를 실행하면 채널 대화를 stdio MCP로 노출합니다. 즉, 다른 에이전트(예: Claude Code)가 OpenClaw의 대화를 MCP를 통해 읽고 쓸 수 있습니다.
...
주목할 점은 Hermes가 `hermes claw migrate` 명령으로 OpenClaw에서의 마이그레이션 도구를 제공한다는 것입니다. SOUL.md, MEMORY.md, 스킬, API 키, 플랫폼 설정까지 옮겨줍니다. 실제로 제가 넘어올 때 이 도구를 사용했고, 예상보다 매끄러웠습니다.
...
Hermes는 **Agent Layer를 깊게 확장합니다**. 스킬 축적, MoA, 서브에이전트 위임, prefix cache 최적화로 에이전트가 매일 더 똑똑해지는 방향을 지향합니다.

OpenClaw는 **Control Layer와 Distribution Layer를 넓게 확장합니다**. 세션 격리, 샌드박스 정책, WebSocket 프로토콜, 40개 채널 플러그인, MCP 서버 모드로 에이전트가 어디서든 안정적으로 운영되는 방향을 지향합니다.

저는 OpenClaw로 시작해 Hermes로 넘어왔고 만족합니다. 하지만 이는 Hermes가 "더 좋아서"가 아니라 **제가 에이전트에게 기대하는 것이 "안정적인 비서"에서 "성장하는 동료"로 바뀌었기 때문입니다.**
...
Hermes Agent와 OpenClaw의 차이는 Agent-first와 Gateway-first의 차이입니다. Hermes는 에이전트의 성장, 메모리, 스킬, 멀티에이전트 구조에 초점을 두고, OpenClaw는 채널과 게이트웨이 운영에 초점을 둡니다.

### 보충 해설

비교할 때 어느 쪽이 더 좋다는 식으로만 보면 안 됩니다. 원하는 문제가 에이전트 자체의 자율성과 성장이라면 Hermes가 더 잘 맞고, 여러 채널을 안정적으로 연결하고 운영하는 것이 목적이라면 OpenClaw의 설계가 더 맞습니다. 선택 기준은 기능 수가 아니라 중심 아키텍처입니다.
