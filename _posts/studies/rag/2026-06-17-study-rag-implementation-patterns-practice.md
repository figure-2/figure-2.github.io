...
RAG를 처음 볼 때는 vector DB에 문서를 넣고 질문하면 되는 구조로 이해했습니다. 그러나 구현 흐름을 단계별로 나눠보니 실제 품질 차이는 그 앞뒤에서 크게 났습니다.

문서를 어떻게 자르는지, 질문을 어떻게 바꾸는지, 검색 결과를 어떻게 재정렬하는지, 무엇을 기준으로 평가하는지에 따라 같은 LLM을 사용해도 결과가 달라집니다.
...
처음부터 query rewrite, HyDE, reranker, agentic retrieval을 모두 넣으면 무엇이 효과가 있었는지 알기 어렵습니다.

저는 아래 순서로 보는 편이 더 낫다고 느꼈습니다.
...
이 순서를 지키면 막연한 체감 개선에서 멈추지 않고 “어느 지점이 좋아졌다”라고 말할 수 있습니다.

## Chunking은 검색 품질의 시작점

chunking은 단순한 전처리가 아닙니다. RAG가 보는 세계를 정하는 작업입니다.
...
제가 가장 중요하게 본 것은 chunk 크기보다 `chunk에 어떤 설명을 붙일 것인가`였습니다. 같은 문장이라도 어느 문서의 어느 section인지 알 수 없으면 답변의 근거로 쓰기 어렵습니다.

## Query rewrite와 HyDE

사용자의 질문은 문서에 있는 표현과 다를 때가 많습니다. 이때 query rewrite가 필요합니다.

예를 들어 사용자가 “이 회사가 돈을 잘 벌고 있어?”라고 물으면 문서에는 “매출액”, “영업이익”, “수익성”, “전년 대비” 같은 표현으로 정보가 흩어져 있을 수 있습니다.
...
다만 query를 늘리면 검색 비용과 noise도 늘어납니다. 따라서 rewrite를 붙인 뒤에는 Top-K 안에 실제 근거가 더 잘 들어오는지 봐야 합니다.

## Reranker를 붙이는 이유

첫 검색 결과는 recall 중심으로 넓게 가져오는 경우가 많습니다. 이때 LLM에 그대로 넣으면 context noise가 생깁니다.

Reranker는 검색된 후보를 다시 읽고 질문에 더 가까운 순서로 재정렬합니다.
...
검색 후보가 많아질수록 reranker의 가치가 커집니다. 반대로 데이터가 작고 질문이 단순하면 reranker가 latency만 늘릴 수도 있습니다.

## 실습에서 볼 지표

RAG 구현을 비교할 때는 답변 하나만 보면 안 됩니다.
...
제가 본 예시 중에는 단순 RAG보다 advanced retrieval을 붙였을 때 점수가 올라간 경우가 있었습니다. 다만 이런 수치는 데이터셋과 평가 방식에 묶여 있으므로 일반 성능처럼 말하면 안 됩니다.

## 검색 단계를 분리한 코드

RAG 검색 흐름을 코드로 적어보면 검색기를 하나로 보지 않게 됩니다. rule 기반 후보 제한, BM25 sparse retrieval, dense retrieval을 순서대로 태웁니다. 검색 품질을 높인다는 말은 결국 후보를 넓히고, 줄이고, 다시 정렬하는 과정을 어떻게 나누느냐의 문제였습니다.

검색 흐름만 떼어내면 다음과 같습니다.
...
여기서 `rule_filter`는 정확도를 보장하는 장치가 아닙니다. 검색 공간을 줄이는 장치입니다. 그 뒤에 sparse, dense, reranker를 분리해야 어디서 실패했는지 추적할 수 있습니다.

## 남은 판단

RAG 구현은 “vector DB를 붙였다”로 끝나지 않습니다. 실제로는 chunking, query, retrieval, reranker, evaluation을 나눠서 봐야 합니다.

검색 결과가 틀렸을 때 LLM부터 바꾸면 원인을 놓치기 쉽습니다. 그 전에 chunk, query, Top-K를 확인하는 습관이 더 중요했습니다.
