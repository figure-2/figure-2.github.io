---
title: "LLM Judge로 요약 품질을 점검할 때 조심해야 할 것"
categories:
- 2.PROJECT
- 2-5. SeSAC-Note
tags:
...
# LLM Judge로 요약 품질을 점검할 때 조심해야 할 것
...
이 구분이 중요하다. Judge 수치를 잘못 쓰면 "품질을 보장했다"처럼 보일 수 있다. 이 글에서는 Summarizer와 Judge를 왜 나눴는지, benchmark 수치를 어떻게 제한해서 해석해야 하는지 정리한다.
...
이 축들은 품질 보증이 아니라 점검 기준이다. LLM Judge 역시 LLM이므로 실수할 수 있다. 따라서 Judge 결과는 자동 검사의 한 종류로 보고, 공개 글에서는 제한된 benchmark 기준으로만 표현한다.
...
팀 프로젝트 기록 기준으로 별도 실험에서는 Judge 통과율이 80%에서 95%로 개선된 사례가 정리되어 있다. 이 수치는 해당 문서의 실험 조건 기준이며, 전체 서비스 품질을 의미하지 않는다.

## 제한된 benchmark 결과

프로젝트 정리 문서에는 Judge v3 benchmark가 다음처럼 기록되어 있다.
...
따라서 공개 글에서는 "benchmark 조건에서 평가 단계 효율 개선이 관찰됐다" 정도로 표현하는 것이 안전하다.

## Judge를 운영 품질 보증으로 말하지 않는 이유

Judge는 품질을 높이는 데 도움을 줄 수 있지만, 품질을 보장하지 않는다. 특히 강의 유형이 달라지면 VLM 입력, STT 품질, Fusion 정확도, 요약 난도도 달라진다.
...
- 이전 글: [단순 챗봇이 아니라 영상 근거 안에서 답하는 QA 흐름 만들기]({% post_url 2026-02-10-project-sesac-note-evidence-qa %})
- 다음 글: [검증 범위, 보안 보강, 그리고 프로젝트 회고]({% post_url 2026-02-10-project-sesac-note-validation-retrospective %})
