...
발표 정리 자료 기준으로 Judge는 기준 점수에 미달한 결과에 대해 피드백을 만들고, 그 피드백을 바탕으로 요약을 보완하거나 재생성하는 loop로 설명된다. 공개 글에서는 이를 최종 판정 장치가 아니라 위험을 줄이는 보조 점검 loop로 제한한다.
...
여기서 `7.0`은 발표 정리 자료 기준의 gate 설명이다. 모든 강의에서 이 기준이 절대적인 품질 기준이라는 의미는 아니다.
