...
Agent 평가는 정답률 하나로 끝나지 않습니다. Lumi_agent처럼 페르소나, 기억, 도구 호출, 승인 경계가 섞인 시스템은 무엇을 평가할지부터 나눠야 합니다.

그래서 이 글은 성능 결과표가 아니라 평가 설계 자체를 다룹니다. 어떤 기준으로 Agent를 봐야 하는지, 그리고 어떤 범위까지 말할 수 있는지를 분리합니다.

## 평가해야 할 것

Lumi_agent의 품질은 최소 네 가지로 나눠 봐야 합니다.
...
이 축을 섞으면 평가가 흐려집니다. 예를 들어 응답 말투가 자연스러워도 메시지 전송 도구를 잘못 호출하면 Agent 품질은 낮습니다.
