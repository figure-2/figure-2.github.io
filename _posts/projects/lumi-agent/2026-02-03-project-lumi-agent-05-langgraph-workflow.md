...
Lumi_agent에서 LangGraph를 쓴 이유는 “Agent를 만들 수 있어서”가 아니라 “실행 흐름을 보이게 만들 수 있어서”입니다.

외부 도구를 호출하는 Agent는 단순히 답변을 생성하는 챗봇보다 실패 지점이 많습니다. 도구를 호출할지, 어떤 도구가 안전한지, 실행 후 다시 생각할지, 마지막에 기억을 저장할지 결정해야 합니다.

## wrapper 방식의 한계

Agent wrapper를 쓰면 빠르게 만들 수 있습니다. 하지만 실행 흐름이 라이브러리 안쪽에 숨으면 다음 질문에 답하기 어려워집니다.
...
Lumi_agent는 이 흐름을 `StateGraph`로 분리했습니다.
