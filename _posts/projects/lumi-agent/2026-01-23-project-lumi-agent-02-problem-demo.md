...
이 요청은 단순 질문이 아닙니다. 검색하고, 결과를 고르고, 내용을 요약하고, 보낼 채널을 찾고, 실제 전송까지 이어지는 작업입니다. 일반 챗봇은 답변을 만들 수 있지만, 이 흐름을 끝까지 실행하려면 Agent 구조가 필요합니다.

## 왜 PC Agent인가

기존 AI 비서는 웹이나 모바일 앱 안에서 잘 동작합니다. 하지만 사용자가 실제로 일하는 PC 환경에서는 도구가 흩어져 있습니다.
...
Lumi_agent의 문제 정의는 “답변을 잘하는 챗봇”이 아니었습니다. 목표는 개인 PC 위에서 사용자의 업무 도구를 연결하고, 필요한 작업을 대화 흐름 안에서 실행하는 Agent 비서였습니다.

## 데모 흐름

데모 흐름은 크게 세 가지로 나눌 수 있습니다.
...
이 흐름의 핵심은 하나의 도구 호출이 아니라 여러 도구가 이어지는 것입니다. 예를 들어 검색, 일정 확인, 메시지 전송이 이어지는 3단계 흐름은 PC Agent가 다뤄야 할 대표적인 멀티홉 작업입니다.
