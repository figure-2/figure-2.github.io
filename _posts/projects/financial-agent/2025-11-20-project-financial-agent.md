...
Financial-Agent는 자연어 질문을 해석해 주식 시장 데이터를 조회하고 분석하는 AI 에이전트입니다. 가격 조회, 복합 조건 검색, 기술적 신호 감지, 모호한 질문의 재작성, 개인 투자 성향을 반영한 위험 관리를 각각 독립된 작업으로 나눴습니다.

<br>

## LangGraph로 작업별 실행 흐름을 분리했다

LangGraph가 상태를 관리하고, 5가지 작업을 하위 그래프로 나눠 실행하도록 설계했습니다.
...
가격, 등락률, 시가총액과 같은 금융 정보를 자연어 질문으로 조회합니다.
...
여러 조건을 AND 연산으로 결합해 조건에 맞는 종목을 찾습니다.
...
RSI, 이동평균선, 볼린저밴드 같은 기술적 분석 지표로 매매 신호를 찾습니다.
...
축약어와 은어를 풀어 쓰고, 날짜나 종목명처럼 빠진 정보는 사용자에게 다시 묻습니다.
...
투자자의 매매 패턴과 마이데이터(투자 성향, 자산 규모)를 분석해 과도한 집중 투자 위험을 경고합니다.
...
LLM이 자연어 질문을 구조화된 JSON으로 바꾸면, 실행 계층이 JSON을 SQL 쿼리로 매핑해 조회합니다.
...
투자자의 성향과 생애주기(나이)를 반영해 개인별 위험 임계값을 계산합니다.
...
1. **에이전트 실행 흐름**: LangGraph로 순환과 분기가 있는 실행 흐름을 설계하며 LLM 애플리케이션의 제어 지점을 나눴습니다.
2. **자연어를 SQL로 변환(Text-to-SQL)**: 자연어 질문을 정확한 SQL로 변환하도록 프롬프트에 스키마 정보를 구성하고, LLM의 환각을 제어하는 프롬프트 작성 방법을 익혔습니다.
3. **금융 규칙 반영**: 은어, 기술적 지표, 위험 관리 이론을 처리 로직에 반영해 금융 질문에 맞는 도구로 발전시켰습니다.
