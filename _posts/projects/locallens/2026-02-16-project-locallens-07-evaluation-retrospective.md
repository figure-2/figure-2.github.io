...
LocalLens의 평가는 두 층으로 나눠 봐야 합니다. 하나는 발표자료 기준의 모델 선택 근거이고, 다른 하나는 코드와 commit history에서 확인되는 실험/테스트 흐름입니다. 두 근거를 섞으면 숫자의 의미가 흐려집니다.

## 발표자료 기준 정량 근거

텍스트 인코더 선택에서는 후보 모델의 점수와 처리 시간이 함께 비교됐습니다.

![텍스트 인코더 후보 비교](/assets/images/locallens/01-text-model-benchmark.png)

발표자료의 MIRACL 기준 후보 비교는 다음과 같습니다.
...
점수만 보면 더 높은 후보가 있었지만, 로컬 검색기에서는 인덱싱 시간이 중요했습니다.

![텍스트 인코더 처리 시간 비교](/assets/images/locallens/02-text-model-latency.png)

발표자료의 500개 문서 처리 실험 기준 처리 시간은 다음과 같습니다.
...
이 수치는 “로컬 검색기에서 왜 경량 모델을 선택했는가”를 설명하는 근거입니다. 다만 특정 하드웨어, 데이터, 실행 조건에 묶인 발표자료 기준이므로 일반 성능 보장처럼 쓰지 않습니다.
