...
SeSAC:Note의 출발점은 "강의 영상을 요약하자"가 아니었습니다. 더 정확한 문제는 "학습자가 영상을 다시 보지 않아도 핵심 개념과 근거를 읽을 수 있게 만들 수 있는가"였습니다.

이 차이가 중요했습니다. 영상 요약이라고만 정의하면 결과물은 짧은 문단 몇 개로 끝나기 쉽습니다. 하지만 학습 노트라고 정의하면 화면, 음성, 시간, 근거, 질의응답까지 함께 설계해야 합니다.
...
강의 영상은 탐색 비용이 큽니다. 30분짜리 영상을 다시 보면서 특정 개념이 나온 구간을 찾는 일은 생각보다 피곤합니다. 학습자는 보통 다음 일을 반복합니다.
...
문제는 강의 내용이 시간축 위에 흩어져 있다는 점입니다. 한 개념을 이해하려면 강사의 말, 슬라이드 제목, 도표, 수식, 예시 코드가 함께 필요할 수 있습니다. 하지만 영상 플레이어는 이 정보를 학습자가 직접 찾게 만듭니다.

또 다른 문제는 학습자의 인지 부담입니다. 온라인 강의 학습자는 시청, 이해, 필기, 구간 탐색을 동시에 해야 합니다. 특히 기술 강의에서는 전문 용어, 수식, 코드, 도표가 빠르게 지나가므로 필기에 집중하다가 설명 맥락을 놓치기 쉽습니다.
...
따라서 이 프로젝트의 문제는 단순히 "요약이 필요하다"가 아니라, 학습자가 영상 안의 근거를 반복해서 직접 찾아야 하는 부담을 줄이는 것이었습니다.

## STT-only 요약의 한계

STT-only 요약은 구현이 간단합니다. 음성을 텍스트로 바꾸고, 그 텍스트를 LLM에 넣으면 됩니다. 하지만 강의에서는 화면에만 존재하는 정보가 많습니다.
...
강사의 음성은 화면을 전제로 합니다. "여기서 중요한 건 이 부분입니다"라는 말은 화면 정보 없이 독립적으로 이해하기 어렵습니다. 그래서 STT만 요약하면 문장은 자연스러워도 학습 근거가 부족한 결과가 나올 수 있습니다.

강의자는 화면의 특정 수식이나 개념을 가리키며 "이것", "여기", "다음 식"처럼 말할 수 있다. STT에는 지시어만 남고, 화면의 실제 수식이나 표는 빠진다. 이 경우 요약 모델은 문맥을 일반 지식으로 메우기 쉬워진다.
...
- 이전 글: [03. 6장으로 보는 SeSAC:Note 포트폴리오 요약]({% post_url projects/sesac-note/2025-12-05-project-sesac-note-03-visual-portfolio %})
- 다음 글: [05. 아키텍처: STT, VLM, Fusion을 연결하는 방법]({% post_url projects/sesac-note/2025-12-16-project-sesac-note-05-architecture %})
