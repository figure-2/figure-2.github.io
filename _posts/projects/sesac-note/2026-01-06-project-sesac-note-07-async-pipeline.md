...
이 타임라인은 성능 수치를 과시하기 위한 것이 아니라, 병목을 발견한 뒤 구조를 어떻게 바꿨는지 보여주는 근거입니다.
...
이 흐름에서 확인할 지표는 전체 처리 완료 시간이 아니라 첫 결과가 화면에 나타나기까지의 시간입니다. 첫 응답 지연의 측정 조건과 개선 과정은 [OCR·요약·RAG 개선 기록]({% post_url projects/sesac-note/2026-09-24-project-sesac-note-optimization-record %})에 정리했습니다.

배치 크기에 따른 전체 요약 실행 시간·토큰 사용량의 비교는 [Summarizer 배치 크기 실험]({% post_url projects/sesac-note/2026-09-24-project-sesac-note-source-summarizer-batch %})에서 다룹니다. 배치 크기를 바꾸는 실험과 결과를 먼저 노출하는 구조는 서로 관련돼 있지만, 같은 성능 지표로 합쳐 해석하지 않습니다.
...
비동기 구조에서 정리한 핵심은 독립 작업의 병렬 실행, 단계별 상태 저장, 완료된 결과의 순차 노출입니다. 이 글에서는 그 구조를 다루고, 배치별 비교표와 첫 응답 개선 결과는 위의 실험·구현 글로 나누어 설명했습니다.
