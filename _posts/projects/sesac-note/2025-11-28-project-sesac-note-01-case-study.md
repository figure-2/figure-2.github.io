...
SeSAC:Note는 강의 영상의 음성 정보와 화면 정보를 시간축으로 결합해, 영상 없이도 복습 가능한 AI 강의 노트와 영상별 질의응답을 제공하는 멀티모달 AI 서비스입니다.
...
강의 영상은 일반 문서와 다릅니다. 핵심 정보가 한 곳에 있지 않고, 음성 설명과 화면 자료가 시간축 위에 흩어져 있습니다.

음성에는 강사의 설명이 있습니다. 화면에는 수식, 표, 도표, 코드, 슬라이드 제목, 강조 표시가 있습니다. STT 결과만 요약하면 음성으로 말하지 않은 화면 정보가 빠집니다. 반대로 화면만 보면 강사가 왜 그 장표를 설명했는지 알기 어렵습니다.

그래서 이 프로젝트의 목표는 단순히 영상을 짧게 요약하는 것이 아니었습니다. 목표는 화면과 음성을 함께 근거로 삼아, 학습자가 영상을 계속 되돌려 보지 않아도 이해 가능한 독립형 강의 노트를 만드는 것이었습니다.

## 해결 구조

SeSAC:Note의 처리 흐름은 다음과 같습니다.
...
먼저 영상에서 음성을 추출하고 STT로 변환합니다. 동시에 의미 있는 화면 변화를 찾아 슬라이드를 캡처합니다. 캡처된 화면은 VLM이 텍스트, 수식, 표, 도표 같은 시각 정보를 구조화합니다. 이후 STT와 VLM 결과를 timestamp 기준으로 묶어 segment를 만들고, 이 segment를 바탕으로 Summarizer가 강의 노트를 생성합니다.

생성된 노트는 Judge가 groundedness와 note quality 관점에서 점검합니다. 마지막으로 사용자는 요약 노트를 읽고, 특정 영상의 summary, segment, evidence 범위 안에서 질문할 수 있습니다.
...
- 다음 글: [02. SeSAC:Note 핵심 기능과 구조]({% post_url projects/sesac-note/2025-12-02-project-sesac-note-02-readme-overview %})
