...
같은 PDF를 5가지 Parser API로 처리하고 결과를 비교했습니다.
...
모든 API에 동일한 PDF를 입력하고 바운딩 박스 정확도와 HTML 추출 품질을 중심으로 평가했습니다. 바운딩 박스는 텍스트나 표 등 문서 요소의 위치와 범위를 나타내는 사각형입니다.

## 1. Upstage Parser API

> **장점: 객체 영역을 일관되게 탐지**
> 
> - **일관성 있는 바운딩 박스**: 문서 요소의 영역을 가장 일관되게 탐지했고, HTML 결과에서도 표 구조를 정확하게 인식했습니다.
> 
...
> **한계: 복잡한 구조 처리**
> 
> - **이중 구조 표 처리**: 복잡한 이중 구조 표에서는 바운딩 박스 정확도가 떨어졌지만, **HTML 결과에서는 중첩표를 정확하게 탐지했습니다.**
> 
...
> **한계: 그래프 인식 부족**
> 
> - **중첩 표 내 그래프**: 중첩 표 안에 있는 그래프를 인식하지 못했습니다.
> 
...
> **한계: 기본 표 처리 부족**
> 
> - **표 처리**: 바운딩 박스가 부정확하며, 단일 표 형식에서도 정확한 추출이 어려웠습니다.
> 
> **원본 PDF 1페이지 - 바운딩 박스 분석 결과**
...
> **중첩표 처리 결과**
> 
> - **표 처리**: 바운딩 박스가 부정확하며 중첩표 구조에서 내용이 한 칸씩 밀렸습니다. **(빨간색 표시)**
> - **확인한 장점**: 중첩표 구조 자체는 정확하게 인식했습니다. **(파란색 표시)**
> 
> **원본 PDF 2페이지 - 중첩표 바운딩 박스 분석**
...
> **한계: 일관성 부족**
> 
> - **표 처리**: 바운딩 박스가 부정확하며 프롬프트와 PDF 파일에 따라 결과가 달라졌습니다.
> - **일관성 문제**: 추출 결과의 일관성과 정확성이 떨어졌습니다.
> - **확인한 장점**: HTML 결과에서는 중첩표를 정확하게 인식하고 복잡한 구조도 잘 처리했습니다.
> 
...
> **장점: 그래프 추출 가능**
> 
> - **그래프 인식**: 바운딩 박스는 부정확했지만 HTML 결과에서는 그래프를 정확하게 탐지하고 추출했습니다.
> 
> **원본 PDF 3페이지 - 그래프 바운딩 박스 분석**
...
Gemini API의 결과는 OpenAI API와 유사했습니다. 차이는 HTML 추출 과정에서 그래프의 내용을 파악해 간단한 설명을 함께 제공한다는 점입니다.
...
> 
> - **그래프 인식**: 바운딩 박스는 부정확했지만 HTML 결과에서는 그래프를 정확하게 탐지하고 추출했습니다.
> - **추가 기능**: 그래프의 내용을 파악해 간단한 설명을 함께 제공했습니다.
> 
> **원본 PDF 3페이지 - 그래프 바운딩 박스 분석**
...
GoogleCloud Document AI API는 GCP 환경과 함께 사용할 수 있다는 장점이 있습니다.

> **한계: 구조적 처리 부족**
> 
> - **표 처리**: 바운딩 박스가 부정확하며, HTML/TXT 추출 결과가 나열식으로 출력됐습니다.
> - **구조 문제**: 테이블을 별도로 정리하지 않아 청킹과 데이터 중복 문제가 예상됐습니다.
> 
> **원본 PDF 1,2페이지 - 바운딩 박스 분석 결과**
...
> **그래프 추출 결과**
> 
> - **그래프 처리**: 바운딩 박스는 부정확하지만 HTML/TXT 추출에서 그래프를 탐지했습니다.
> - **데이터 문제**: 추출된 데이터와 누락된 데이터가 혼재되어 있었습니다.
> - **구조적 한계**: 나열식 출력으로 인한 청킹과 데이터 중복 문제가 지속됐습니다.
> 
> **원본 PDF 3페이지 - 그래프 바운딩 박스 분석**
...
1. **바운딩 박스 정확도**: 복잡한 구조에서는 모든 API의 정확도가 떨어졌습니다.
2. **일관성 문제**: LLM 기반 API들은 프롬프트와 파일에 따라 결과가 변동했습니다.
3. **그래프 처리**: 대부분의 API에서 그래프 인식이 상대적으로 약했습니다.
