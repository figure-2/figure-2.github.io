...
발표 정리 자료 기준으로 Smart Capture는 다음처럼 더 구체적인 기준을 갖는다. 수치는 해당 발표 정리 조건의 설명으로만 사용한다.
...
발표 정리 자료 기준으로 VLM은 Qwen 계열 모델을 사용해 캡처된 슬라이드의 텍스트, 표, 수식, 다이어그램 관계를 구조화하는 역할을 맡았다. 단일 이미지 요청만 반복하면 병목이 커지므로, 여러 캡처 이미지를 batch 단위로 묶어 처리하는 방향도 함께 정리됐다. 이 역시 전체 서비스 성능 주장이라기보다 VLM 병목을 줄이기 위한 설계 선택으로 보는 것이 맞다.
